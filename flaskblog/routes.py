...
from flaskblog import app, db, brcypt, mail
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f' Your Account has been creted . You can login now', 'success')
...
If you did  not make this request please ignoe the email and no change will be applied
'''
    mail.send(msg)
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        flash(f' Your Account has been creted . You can login now', 'success')
        return redirect(url_for('login'))
    return render_template('rest_token.html', title='Reset Pasword', form=form)
